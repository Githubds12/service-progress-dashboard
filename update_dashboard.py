...
<head>
    ...
    <style>
        ...
        * {{ box-sizing: border-box; margin: 0; padding: 0; outline: none; }}
        ...
        .progress-bar-fill {{ height: 100%; background: linear-gradient(to right, #ff0000, #ff7f00, #ffff00, #00ff00, #0000ff, #4b0082, #8f00ff); background-size: 200% auto; animation: rainbow 2s linear infinite, working 2s ease-in-out infinite; width: 0%; transition: width 1s cubic-bezier(0.4, 0, 0.2, 1); box-shadow: 0 0 15px var(--success); position: relative; transform-origin: left; }}
        
        @keyframes working {{
            0%, 100% {{ transform: scaleX(1); opacity: 0.9; }}
            50% {{ transform: scaleX(1.02); opacity: 1; filter: brightness(1.2); }}
        }}
